...
- **API Provider সিস্টেম end-to-end যাচাই সম্পন্ন**: টেস্ট প্রোভাইডার তৈরি করে ৩টি fulfillment mode-ই যাচাই করা হয়েছে — (১) `api` mode + প্রোভাইডার সফল → অর্ডার অটো-কমপ্লিট + ওয়ালেট সঠিকভাবে ডেবিট, (২) `api` mode + প্রোভাইডার ফেইল → অর্ডার অটো-রিফান্ড + ওয়ালেট ব্যালেন্স সঠিকভাবে ফিরে আসে, (৩) `hybrid` mode + প্রোভাইডার ফেইল → অর্ডার `pending`-এ থাকে ম্যানুয়াল রিভিউর জন্য (অটো-রিফান্ড হয় না, এটাই সঠিক ডিজাইন)। তিনটি পাথই সঠিক পাওয়া গেছে, কোনো বাগ নেই।

## Polish Pass — Promo/Notice Modal Blocking Login + WhatsApp FAB/Sidebar Overlap (2026-08-28)
"সেরা" (সবচেয়ে ভালো) করার অনুরোধে সম্পূর্ণ সাইট আবার ভিজ্যুয়ালি অডিট করা হয়েছে (Playwright স্ক্রিনশট, একাধিক ভিউপোর্টে)।

- **গুরুত্বপূর্ণ বাগ পাওয়া গেছে ও ফিক্স করা হয়েছে**: Live Notice bar ও Promo/Offer পপআপ (`syncLiveNoticeBar()`/`syncPromoCard()`) আগে কেবল অ্যাডমিন/স্টাফ রোল চেক করত, কিন্তু কারেন্ট পেজ/রুট চেক করত না — ফলে `/login` ও `/register` পেজেও (যেখানে গেস্ট ইউজার লগইন/রেজিস্ট্রেশন করছে) প্রোমো পপআপ (`fixed inset-0 z-[85]` ফুল-ভিউপোর্ট ওভারলে) দেখানো হত, এবং এটি পুরো লগইন/রেজিস্ট্রেশন ফর্মের উপর পয়েন্টার-ইভেন্ট ব্লক করে রাখত — নতুন ইউজার প্রোমো বন্ধ না করা পর্যন্ত লগইন বাটনেই ক্লিক করতে পারত না! Playwright দিয়ে UI-চালিত লগইন অটোমেশনের সময় টাইমআউট এরর থেকে এই বাগ আবিষ্কৃত হয়। **ফিক্স**: এখন `/login` বা `/register` পেজে থাকলে বার/মোডাল কখনো দেখানো হয় না।
- **আরেকটি ওভারল্যাপ বাগ পাওয়া গেছে ও ফিক্স করা হয়েছে**: মোবাইলে WhatsApp FAB বাটন (bottom-left, `z-[70]`) সাইডবার ওপেন করলে সাইডবারের নিজের "সাহায্য প্রয়োজন?" সাপোর্ট কার্ডের ঠিক উপরে বসে যেত (একই কর্নারে)। **ফিক্স**: এখন সাইডবার খোলা থাকলে FAB হাইড হয়ে যায়, বন্ধ করলে ফিরে আসে।
- **যাচাই করা হয়েছে কিন্তু ফিক্স লাগেনি (আগে থেকেই ঠিক ছিল)**: প্রোমো CTA বাটনের কনট্রাস্ট (WCAG রেশিও ৫.৮৩–১২.০৮:১, AA পাস), ট্যাবলেট টপবার সার্চবক্সের প্রস্থ (৭৬৮-১০০০px রেঞ্জে ওয়ালেট-ব্যালেন্স পিলের সাথে কোনো ওভারল্যাপ নেই)।
- **ডিপ্লয়মেন্ট**: বিল্ড + কমিট (`640fd50`) + GitHub push + Cloudflare Pages প্রোডাকশন ডিপ্লয় সম্পন্ন; প্রোডাকশনে curl দিয়ে নতুন JS মার্কার কনফার্ম করা হয়েছে।
- **ড্যাশবোর্ড ক্লিক কমফোর্ট যোগ করা হয়েছে**: প্রতিটি ক্লিকযোগ্য এলিমেন্টে (সাইডবার নেভ-লিংক, লিস্ট-রো/`spot-card`, পেজিনেশন বাটন, আইকন বাটন — নোটিফিকেশন/ইউজার-মেনু/সাইডবার-টগল/কপি/এডিট/ডিলিট) `:active` স্টেটে তাৎক্ষণিক scale-down (press) ফিডব্যাক যোগ করা হয়েছে (`transform .12s`), এবং মোবাইলে ডিফল্ট গ্রে ট্যাপ-হাইলাইট ফ্ল্যাশ (`-webkit-tap-highlight-color`) বন্ধ করে কাস্টম ফিডব্যাক দিয়ে রিপ্লেস করা হয়েছে। ছোট আইকন বাটনগুলোর (২৮px, যেমন কপি-অ্যাকাউন্ট বাটন) ইনভিজিবল `::before` হিট-এরিয়া প্যাডিং যোগ করে মোবাইলে আরামদায়ক ট্যাপ-টার্গেটের কাছাকাছি আনা হয়েছে — ভিজ্যুয়াল সাইজ অপরিবর্তিত রেখে। CSS-only, কোনো মার্কআপ/JS পরিবর্তন হয়নি, `prefers-reduced-motion` রেসপেক্ট করে। কমিট (`27be19e`) + push + প্রোডাকশন ডিপ্লয় সম্পন্ন; প্রোডাকশনে curl দিয়ে কনফার্ম করা হয়েছে।

## Security Fix + Bug Fix + New Feature: Demo Credentials Removed, Document Download Fixed, Storage Management (2026-08-28)
ইউজার-রিপোর্ট করা তিনটি ইস্যু ফিক্স করা হয়েছে:

- **সিকিউরিটি ফিক্স**: লগইন পেজের ফুটারে পাবলিকলি "ডেমো এডমিন: 01700000000 / Admin@12345" দেখানো হচ্ছিল। D1 কোয়েরি করে নিশ্চিত করা হয়েছে এটি আসলে **রিয়েল এডমিন অ্যাকাউন্ট** (id=1, email: iwantuuu529@gmail.com) — কোনো ডেমো/প্লেসহোল্ডার নয়। এই ক্রেডেনশিয়াল ডিসক্লোজার ব্লক সম্পূর্ণভাবে সরিয়ে ফেলা হয়েছে।
- **বাগ ফিক্স — অ্যাডমিন ডকুমেন্ট ডাউনলোড**: অ্যাডমিন ইউজারের আপলোড করা অর্ডার-ফর্ম ডকুমেন্ট (NID স্ক্যান, ছবি ইত্যাদি) দেখতে/ডাউনলোড করতে পারছিলেন না — কারণ ইউজার-facing অর্ডার API রেসপন্সে `form_schema` ছিল না, এবং ফ্রন্টএন্ড কোড (admin.js ও orders.js) সব `form_data` ভ্যালুকে প্লেইন টেক্সট হিসেবে রেন্ডার করত, ফাইল-টাইপ ফিল্ড স্পেশাল-কেস করত না — ফলে R2 অবজেক্ট-কী (যেমন `orders/5/...-nid_front.jpg`) স্ক্রিনে একটি ক্লিক-অযোগ্য স্ট্রিং হিসেবে দেখাত। **ফিক্স**: নতুন দুটি স্ট্রিমিং এন্ডপয়েন্ট (`GET /api/admin/orders/:id/upload/:fieldName`, `GET /api/orders/:id/upload/:fieldName`) যোগ করা হয়েছে, এবং ফ্রন্টএন্ডে ফাইল-টাইপ ফিল্ড এখন একটি সঠিক "দেখুন/ডাউনলোড করুন" লিংক দেখায় — অ্যাডমিন প্যানেল ও ইউজারের নিজের অর্ডার-ডিটেইল পেজ, দুই জায়গাতেই।
- **নতুন ফিচার — ডকুমেন্ট স্টোরেজ ম্যানেজমেন্ট**: অ্যাডমিন সেটিংসে নতুন "ডকুমেন্ট স্টোরেজ" ট্যাব যোগ করা হয়েছে — R2 বাকেটের (`webapp-files`) মোট ব্যবহার ও ক্যাটাগরি-ভিত্তিক ব্রেকডাউন (ইউজার অর্ডার ডকুমেন্ট / এডমিনের ফলাফল ফাইল / রিচার্জ পেমেন্ট প্রুফ) দেখায়, এবং একটি নিরাপদ "অব্যবহৃত ফাইল মুছুন" বাটন আছে। **অরফান-ডিটেকশন লজিক**: প্রতিটি R2 অবজেক্টকে `orders.result_file_key`, `recharge_requests.proof_file_key`, এবং সব `orders.form_data` JSON-এর ফাইল-কী ভ্যালুর সাথে ক্রস-রেফারেন্স করা হয় — যে ফাইলের সাথে ডাটাবেসে কোনো রেফারেন্স নেই (যেমন টেস্ট অর্ডার বা ডিলিট হওয়া রিকোয়েস্টের পুরনো ফাইল) সেটিকেই "অব্যবহৃত" ধরা হয়; **কোনো সক্রিয় ইউজার-ডকুমেন্ট এই পার্জে প্রভাবিত হয় না**। ডিলিট অ্যাকশন `admin_logs`-এ অডিট-লগ হয়। ব্যাকএন্ড: নতুন `src/routes/admin/storage.ts` (`GET /usage`, `DELETE /orphaned`)।
- **টেস্টিং**: লোকালি PM2 + wrangler dev দিয়ে যাচাই করা হয়েছে — লগইন পেজে ডেমো টেক্সট নেই, `/api/admin/storage/usage` সঠিক JSON রিটার্ন করে (কাউন্ট/সাইজ ব্রেকডাউন সহ), এবং একটি টেস্ট অর্ডারের `documents` ফাইল-ফিল্ড থেকে নতুন ডাউনলোড এন্ডপয়েন্ট সফলভাবে PDF স্ট্রিম করেছে।
- **ডিপ্লয়মেন্ট**: বিল্ড + কমিট (`ae2f593`) + GitHub push + Cloudflare Pages প্রোডাকশন ডিপ্লয় সম্পন্ন; প্রোডাকশনে curl দিয়ে নিশ্চিত করা হয়েছে লগইন পেজে ডেমো ক্রেডেনশিয়াল টেক্সট নেই এবং নতুন স্টোরেজ API এন্ডপয়েন্ট সঠিকভাবে ৪০১ (auth-protected) রিটার্ন করছে।

## D1/R2 Test-Data Cleanup + Narrow-Mobile UI Verification (2026-08-28)

- **D1 প্রোডাকশন টেস্ট-ডেটা ক্লিনআপ**: প্রোডাকশন ডাটাবেস অডিট করে একটি স্পষ্ট টেস্ট অ্যাকাউন্ট (user id=2, নাম "fcgfcg", ব্যালেন্স ৯,৯৯,৯৯,০৫০ টাকা — একটি ১০-কোটি-টাকার টেস্ট "admin_adjustment" থেকে) চিহ্নিত করা হয়েছে এবং ব্যবহারকারীর অনুমোদনক্রমে স্থায়ীভাবে মুছে ফেলা হয়েছে, সহ তার সাথে যুক্ত সব ডেটা: ১টি টেস্ট অর্ডার + ৩টি order_logs এন্ট্রি, ৩টি ওয়ালেট ট্রানজেকশন, ১টি রিচার্জ রিকোয়েস্ট, ৬টি নোটিফিকেশন, এবং ৫টি সংশ্লিষ্ট admin_logs এন্ট্রি — সঠিক ফরেন-কী নির্ভরতার ক্রমে ডিলিট করা হয়েছে যাতে কোনো orphan রেফারেন্স না থাকে। এছাড়া R2 বাকেট (`webapp-files`) থেকে ৩টি অবশিষ্ট orphaned টেস্ট ফাইল (কোনো অর্ডারের সাথে যুক্ত নয়) সরাসরি Cloudflare API দিয়ে মুছে ফেলা হয়েছে — নতুন "ডকুমেন্ট স্টোরেজ" ফিচার তৈরি করার সময়ই এগুলো ম্যানুয়ালি চিহ্নিত হয়েছিল। **আসল ইউজার অ্যাকাউন্ট** (id=1 এডমিন, id=4 "MD RAZ", id=5 "hpt") এবং তাদের ডেটা স্পর্শ করা হয়নি। ক্লিনআপের পর প্রোডাকশন D1: orders/transactions/recharge_requests সব ০, R2 বাকেট সম্পূর্ণ খালি (০ অবজেক্ট) — অ্যাপ স্বাভাবিকভাবে কাজ করছে (হোম/লগইন পেজ ২০০ কনফার্মড)।
- **Narrow-mobile (320px / 360px / 375px) ভিজ্যুয়াল যাচাই**: Playwright দিয়ে প্রোডাকশন সাইটে (landing, login, register) তিনটি সাধারণ ন্যারো ভিউপোর্টে (iPhone SE ৩২০px, সাধারণ Android ৩৬০px, iPhone 12/13 mini ৩৭৫px) স্ক্রিনশট + `document.scrollWidth` বনাম `window.innerWidth` তুলনা করে যাচাই করা হয়েছে — **তিনটি সাইজেই কোনো হরাইজন্টাল স্ক্রল/ওভারফ্লো পাওয়া যায়নি** (`scrollWidth === innerWidth`)। স্ক্রিনশটে টেক্সট-ওভারল্যাপ, কাটা বাটন বা ভাঙা লেআউট কিছুই দেখা যায়নি — ফর্ম ইনপুট, বাটন, কার্ড সবই সঠিকভাবে wrap/stack হয়েছে। ডেকোরেটিভ aurora ব্যাকগ্রাউন্ড ব্লব ও live-notice মার্কি-ট্র্যাক DOM-এ ভিউপোর্টের বাইরে বিস্তৃত থাকলেও এগুলো প্যারেন্ট `overflow:hidden` দিয়ে ক্লিপ করা, তাই প্রকৃত পেজ-স্ক্রল ঘটে না — এটি প্রত্যাশিত/ইচ্ছাকৃত আচরণ, বাগ নয়।

## Not Yet Implemented / Next Steps
- ~~Real UddoktaPay (or other) auto payment-gateway webhook/callback integration~~ — **DONE (2026-09-12)**: সম্পূর্ণ UddoktaPay অটো-রিচার্জ লুপ ইমপ্লিমেন্টেড। `POST /api/wallet/recharge/auto` গেটওয়ের `/api/checkout-v2` (header `RT-UDDOKTAPAY-API-KEY`) এ checkout খোলে, `gateway_payments` + অটো `recharge_requests` row রাখে এবং ক্রেতাকে পেমেন্ট পেজে পাঠায়; ফিরে এলে (`?pay=success&invoice_id=...`) বা webhook (`POST /api/webhooks/uddoktapay`) এলে `POST /api/wallet/recharge/auto/verify` গেটওয়ের `/api/verify-payment` দিয়ে pull-verify করে এবং এডমিন-অ্যাপ্রুভালের হুবহু একই পাথে (first-recharge referral bonus সহ) ব্যালেন্স ক্রেডিট করে। ওয়ালেট UI-তে "অটো রিচার্জ — সাথে সাথে ব্যালেন্স" কার্ড + কুইক অ্যামাউন্ট বাটন। `payment_gateways` রো self-heal: untouched রো-তে স্যান্ডবক্স কী + active; এডমিনের সেট করা live কী কখনো overwrite হয় না। Sandbox দিয়ে end-to-end verified (checkout URL + invoice_id + verify API সফল; গেটওয়ের পেমেন্ট পেজটি headless browser-এ শুধু Cloudflare bot-gate এর কারণে আটকায় — ইন্টিগ্রেশনের বাইরের বিষয়)।
...
## Fulfillment Logic Update (2026-09-10): সব সার্ভিসের প্রপার প্রসেসিং লজিক

### সমস্যা ছিল
১১টি সার্ভিসের মধ্যে ১০টিই ছিল `manual` — অর্ডার দিলেই শুধু এডমিন কিউতে পড়ে থাকত, আর অটো-লজিক ছিল একটিমাত্র হার্ডকোডেড স্লাগ চেকে (`slug === 'nid-create'`)।

### এখন যা আছে — কনফিগ-ড্রিভেন ফুলফিলমেন্ট ইঞ্জিন (`src/services/fulfillment.service.ts`)
প্রতিটি সার্ভিসের নিজস্ব প্রসেসিং ফ্যামিলি, কোনো হার্ডকোডেড স্লাগ-চেক ছাড়াই:

| ফ্যামিলি | সার্ভিস | লজিক |
|---|---|---|
| **অটো ডকুমেন্ট** (⚡ ইনস্ট্যান্ট) | `nid-create`, `nid-make`, `nibandan-pdf-create` | ইউজারের আপলোড করা ডকুমেন্ট/ডাটা থেকে ডিজাইনার কার্ড তৈরি করে → অর্ডার সাথে সাথে `completed`, স্ট্রাকচারড `result_data` (টাইপ `nid_card`) |
| **লুকআপ / ভেরিফিকেশন** (🔄 হাইব্রিড) | `birth-certificate-search`, `birth-ministry-data`, `nid-sign-copy`, `land-dakhila-finder` | সঠিক `field_mapping` সহ অ্যাডমিন-কনফিগারড **অ্যাক্টিভ** API প্রোভাইডারে ডিসপ্যাচ; প্রোভাইডার না থাকলে নিরাপদে ম্যানুয়াল কিউতে ফলব্যাক |
| **লুকআপ (API)** | `nid-server-copy` | প্রোভাইডার কল; ব্যর্থ হলে অটো-রিফান্ড |
| **ম্যানুয়াল** (👤) | `birth-registration-application`, `nid-user-pass`, `nid-smart-card-pdf`, `custom-request` | এগুলোর সঠিক লজিক-ই হলো হিউম্যান প্রসেসিং |

**গুরুত্বপূর্ণ নীতি:** সরকারি ডাটাবেস-লুকআপ সার্ভিসগুলো কখনো ফেক/বানোয়াট ডাটা তৈরি করে না — রেজাল্ট আসে শুধু কনফিগারড প্রোভাইডার থেকে, নয়তো ম্যানুয়াল রিভিউ।

### মাইগ্রেশন `0003_service_logic.sql`
বিদ্যমান ডেটাবেজে মোড + `field_mapping` আপডেট করে। লোকাল d1Adapter এখন স্টার্টআপে ০০১-পরবর্তী সব মাইগ্রেশন অটো-অ্যাপ্লাই করে (ইডেমপোটেন্ট)।

### ফ্রন্টএন্ড
- সার্ভিস কার্ড/ডিটেইলে প্রসেসিং টাইপ ব্যাজ (`ইনস্ট্যান্ট অটো / অটো + ম্যানুয়াল / ম্যানুয়াল`)।
- অর্ডার ডিটেইলে টাইপ-অ্যাওয়ার রেজাল্ট রেন্ডারার: `nid_card` → ছবি + স্বাক্ষর + ফিল্ডসহ ডকুমেন্ট প্রিভিউ; `auto_document` → ফিল্ড গ্রিড + ফাইল লিংক; অন্যথায় জেনেরিক কি/ভ্যালু।

### টেস্ট (পাস)
- `nid-make` + `nid-create` অর্ডার → সাথে সাথে `completed`, `nid_card` result_data ✓
- `birth-certificate-search` (প্রোভাইডার ইনঅ্যাক্টিভ) → `pending`, এডমিন কিউ + নোটিফিকেশন ✓
- বিল্ড + ফ্রন্টএন্ড সিনট্যাক্স চেক ✓

## BDRIS Live API Integration (2026-09-10): জন্ম নিবন্ধন যাচাই এখন সত্যিকারের অটো

### যা যুক্ত হয়েছে
`birth-certificate-search` সার্ভিস এখন **SkSeba BDRIS 2-step captcha API**-র সাথে লাইভ কানেক্টেড — সরকারি ডাটাবেস থেকে আসল রেকর্ড আসে, প্ল্যাটফর্ম কখনো ডাটা বানায় না।

**ফ্লো:**
1. ইউজার অর্ডার দেয় (১৭ ডিজিট জন্ম নিবন্ধন নম্বর + জন্ম তারিখ)
2. ইঞ্জিন সাথে সাথেই প্রোভাইডারের `/api/v2/bdris/captcha` কল করে (Step 1) → সেশন + ক্যাপচা ইমেজ
3. অর্ডার `processing` অবস্থায় অপেক্ষা করে; ইউজার অর্ডার ডিটেইলে ক্যাপচা ছবি দেখে কোড লেখে
4. `/api/v2/bdris/verify` (Step 2) কল হয় → সফল হলে **অফিসিয়াল রেকর্ডসহ অর্ডার সম্পন্ন** (`bdris_result`)
5. ভুল/মেয়াদোত্তীর্ণ ক্যাপচা → অটো নতুন ক্যাপচা ইস্যু হয়; টার্মিনাল ব্যর্থতা (যেমন রেকর্ড নেই) → ম্যানুয়াল কিউ + এডমিন রিফান্ড/রিজেক্ট

**ভুল স্লাগ/ডাটা হলে** (প্রোভাইডার স্টার্ট ব্যর্থ) → হাইব্রিড সেফটি: নিরাপদে ম্যানুয়াল কিউতে ফলব্যাক।

### নতুন ফাইল/এন্ডপয়েন্ট
- `src/lib/bdris.ts` — API ক্লায়েন্ট (টাইমআউট, এরর হ্যান্ডলিং, ক্যাপচা ইমেজ ফেচ)
- `GET/POST /api/orders/:id/bdris-captcha | bdris-verify` (ইউজার, ওনার-স্কোপড)
- `GET/POST /api/admin/orders/:id/bdris-captcha | bdris-verify` (এডমিন, কাস্টমারের পক্ষে সমাধান)
- অর্ডার ডিটেইলে ক্যাপচা-সলভার কার্ড (ইউজার + এডমিন দুই প্যানেলে), `bdris_result` রেন্ডারার (নেস্টেড রেকর্ড গ্রিড)
- সেটিংস: `bdris_api_key` (মাইগ্রেশন 0004; এডমিন → সেটিংস থেকে পরিবর্তনযোগ্য)

### টেস্ট (পাস)
- লাইভ API রিচযোগ্য, কী অথেনটিকেটেড ✓
- ক্যাপচা ইমেজ প্রক্সি (নিজস্ব ওরিজিন থেকে সার্ভ) ✓
- ভুল ক্যাপচা → নতুন ক্যাপচা পাথ ✓, টার্মিনাল এরর → ম্যানুয়াল কিউ ✓
- ফেক ডাটা কখনো ফেক রেজাল্ট দেয় না ✓
