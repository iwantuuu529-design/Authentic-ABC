...
export function createLocalD1Database(dbFilePath?: string): D1Database {
  ...
  if (isNew) {
    ...
    // Ensure the new NIBANDAN PDF CREATE service is present even if DB was previously initialized
    try {
      const check = db.prepare("SELECT id FROM services WHERE slug = 'nibandan-pdf-create'").get()
      if (!check) {
        console.log('[D1 Adapter] Auto-inserting nibandan-pdf-create service...')
        db.prepare(`
      ...
      console.error('[D1 Adapter] Error ensuring nibandan service:', e)
    }
  }
